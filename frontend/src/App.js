  ...
  return (
    <div className="grid-container">
          ...
          <ul>
            {data.failing_endpoints.map(item => (
              <li key={`${item.service}-${item.path}`}>
                <strong>{item.path}</strong> ({item.service}): {item.count} errors
              </li>
            ))}
          </ul>
      ...
      <div className="grid-item">
        ...
        {data.busiest_period ? (
           <p><strong>{data.busiest_period.minute}</strong> with <strong>{data.busiest_period.request_count}</strong> requests.</p>
        ) : <p>Not enough data.</p>}
      </div>
    </div>
  );
